...
exports.getPostcssPlugins = function ({ designWidth, deviceRatio, postcssOption = {} }) {
    ...
    const cssModulesOption = util_1.recursiveMerge({}, defaultCssModulesOption, postcssOption.cssModules);
    ...
    Object.entries(postcssOption).forEach(([pluginName, pluginOption]) => {
        ...
        if (!util_1.isNpmPackage(pluginName)) { // local plugin
            pluginName = path.join(util_1.appPath, pluginName);
        }
        try {
            const pluginPath = resolve_1.sync(pluginName, { basedir: util_1.appPath });
